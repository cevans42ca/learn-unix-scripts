...
Takes X and Y values on stdin separated by space, comma, or tab, one pair per
line, then allows predictions based on that data.  If the X values are epoch
times and --find-time-given is specified, the output will be a formatted date
based on the default output of the date command.
EOL
}
